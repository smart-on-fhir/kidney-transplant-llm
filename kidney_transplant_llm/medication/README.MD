...
1. Obtain high quality medication data from EHR  
  ...
  * LLM chart review   
    * [medicationrequest.py](medicationrequest.py) is a generic PyDantic class for this task
      * Allow specific drug class/ingredients to extend this base functionality with specific rules as needed
...
**Notice** 
* FHIR Timing CodeableConcepts [DataType](http://hl7.org/fhir/R4/datatypes.html#Timing) are usually NULL.
* FHIR Timing "text" (and display) are more reliable but still not certain

_Example:_ **non-existent** in real world BCH data but here is the ideal contents of `timing`:   
```json 
{
  "repeat": {
    "frequency": 2,
...
_Example_: **rarely** present and also can't be assumed to exist 
```json 
{
  "code": "229799001",
