...
def parse_date(text:str) -> Optional[datetime]:
    date_list = list_dates(text)
    return None if date_list is None else date_list[0]

def list_dates(text: str) -> List[datetime]:
    """
    :param text: clinical text to search for dates
    :return: List of dates (without character positions)
    """
    with_charpos = list_dates_with_charpos(text)
    date_list = list()
    for entry in with_charpos:
        date_list.append(entry[0])
    return date_list

def list_dates_with_charpos(text: str) -> List[Tuple[datetime, Tuple[int, int]]]:
    """ Given clinical text expected to contain dates. Method parse dates from text using regex patterns
